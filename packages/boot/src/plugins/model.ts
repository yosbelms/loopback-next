...
class Model extends PluginBase {
  constructor(options) {
    super(options, 'models', 'model-config');
  }
  ...
  buildInstructions(context, rootDir, modelsConfig) {
...
function buildAllModelInstructions(rootDir, modelsConfig, sources,
  ...
  const modelNamesToBuild = addAllBaseModels(registry, Object.keys(modelsConfig));
